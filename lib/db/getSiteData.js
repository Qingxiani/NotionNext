...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || 'POZE BLOG'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || '免责声明：部分博文引自他人，请关注原创，珍惜优秀作者~（博文仅供个人学习交流，若有相关侵权，请联系我，立即删除'
  const defaultPageCover = NOTION_CONFIG?.HOME_BANNER_IMAGE || '/bg_image3.jpg'
  const defaultIcon = NOTION_CONFIG?.AVATAR || '/touxiang.jpg'
